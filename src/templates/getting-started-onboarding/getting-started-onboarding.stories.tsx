...
import { Accordion, Badge, Button, Container, Text, Title, ProgressBar } from '../../index';

export default {
	title: 'Templates/Onboarding/Getting Started/Page',
	parameters: {
		layout: 'fullscreen',
	},
	tags: [ 'autodocs' ],
...
const Template = ( args: TemplateArgs ) => {
	...
	return (
		<div className="bg-background-secondary min-h-screen w-full">
			<Container containerType="flex" className="min-h-screen justify-center p-8">
				<div className="w-full max-w-4xl mx-auto space-y-8">
					{ /* Header Section */ }
					<div className="bg-background-primary p-8 rounded-xl border border-border-subtle shadow-sm">
						<div className="space-y-4">
							<Text size={ 18 } color="secondary" weight={ 500 }>
					...
					{ allCompleted && (
						<div className="bg-background-primary p-8 rounded-xl shadow-lg relative overflow-hidden">
					...
					{ /* Accordion Section */ }
					<div className="bg-background-primary border border-border-subtle shadow-sm rounded-xl overflow-hidden">
							...
							{ onboardingItems.map( ( item ) => {
								...
								return (
									<Accordion.Item key={ item.id } value={ item.id } className="border-0 overflow-hidden [&:hover_h3]:bg-transparent">
										<Accordion.Trigger
											iconType="arrow"
											className="group p-6 hover:bg-transparent"
										>
											<div className="flex items-center gap-3 flex-1">
												{ isCompleted ? (
													<CheckCircle className="flex-shrink-0 size-6 stroke-support-success" />
												) : (
													<Circle className="peer flex-shrink-0 size-6 group-hover:stroke-background-brand" />
												) }
										...
										<Accordion.Content className="px-6 pb-6 border-0">
											<div className="space-y-6">
												...
												<div className="flex flex-wrap justify-between items-start gap-3 pt-4">
															...
															if ( cta.href ) {
																return (
																	<Text
																		...
																		weight={ 600 }
																		color="primary"
																		className="px-4 py-2 bg-button-primary text-text-on-color rounded-md hover:bg-button-primary-hover transition-colors"
																	>
																		{ cta.label }
																	</Text>
																);
													...
													<div className="flex-shrink-0">
														{ item.ctaButtons.some( ( cta ) => cta.label === 'Mark as Done' ) && (
															<Button
																...
																icon={ isCompleted ? <X className="size-4" /> : <Check className="size-4" /> }
					...
					{ /* Footer Section */ }
					<div className="bg-background-primary p-6 rounded-xl border border-border-subtle shadow-sm">
						<div className="flex items-center justify-between flex-wrap gap-4">
...
export const GettingStartedOnboardingPage = Template.bind( {} );
