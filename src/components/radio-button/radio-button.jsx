	...
	const renderLabel = useCallback( () => {
		...
		return (
			<div className="space-y-1.5">
				...
				{ ( label.description ) ? <p className="text-text-secondary text-sm font-normal leading-5 m-0">{ label.description }</p> : '' }
			</div>
		);
	}, [ label ] );
	...
	return (
		<div className={ cn( 'inline-flex items-center', !! label && 'items-start' ) }>
			<label className={ cn( 'relative flex items-center rounded-full', ! isDisabled && 'cursor-pointer' ) } htmlFor={ radioButtonId }>
				<input ref={ ref } id={ radioButtonId } type="radio" className={ cn( "peer relative cursor-pointer appearance-none transition-all m-0 before:content-[''] checked:before:content-[''] checked:before:hidden before:hidden !border-1.5 border-solid rounded-full", colorClassNames[ color ].checkbox, sizeClassNames[ size ].checkbox, isDisabled && disabledClassNames.checkbox ) } name={ name } value={ value } onChange={ onChange } checked={ checkedValue } disabled={ isDisabled } { ...props } />
				<span className={ cn( 'pointer-events-none inline-flex items-center absolute top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 text-white opacity-0 transition-opacity peer-checked:opacity-100', colorClassNames[ color ].icon, isDisabled && disabledClassNames.icon ) }>
					<div className={ cn( 'rounded-full bg-current', sizeClassNames[ size ]?.icon ) } />
				</span>
			</label>
			{ !! label && (
				<label className={ cn( 'ml-3', ! isDisabled && 'cursor-pointer' ) } htmlFor={ radioButtonId }>
					{ renderLabel() }
				</label>
			) }
		</div>
