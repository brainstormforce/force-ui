...
const DatePickerComponent = ({
  ...
  function CustomMonthCaption(customMonthProps) {
    ...
    const yearFormatted = format(customMonthProps.calendarMonth.date, 'yyyy');
    ...
    const handleYearClick = (yearValue) => {
      ...
      goToMonth(
        new Date(yearValue, customMonthProps.calendarMonth.date.getMonth())
      );
    };
    ...
    return (
      <>
        ...
        {showYearSelect && (
          <div className="grid grid-cols-4 w-full">
            {generateYearRange(yearRangeStart).map((yearValue) => (
              ...
              >
                {yearValue}
                {yearValue === new Date().getFullYear() && currentTimeDot()}
              </Button>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-4 gap-2 my-12">
            {Array.from({ length: 12 }, (_, monthIndex) => (
              <Button
                ...
                onClick={() => {
                  setShowMonthSelect(false);
                  goToMonth(new Date(selectedYear, monthIndex));
                }}
                className={cn(
                  'px-1.5 py-2 h-9 w-[70px] text-center font-normal relative',
                  monthIndex === customMonthProps.calendarMonth.date.getMonth() &&
        ...
        {!showMonthSelect && !showYearSelect && <MonthSelectors weekdays={weekdays} />}
      </>
    );
  }
  ...
  const CustomDayButton = ({ day, modifiers, onSelect }) => {
    ...
    return (
      ...
      >
        {format(day.date, 'd')}
        {isToday && <span className="absolute h-1 w-1 bg-blue-500 rounded-full bottom-1"></span>}
      </button>
    );
  };
  ...
  const Day = (dayProps) => {
    ...
    return (
      <td className={className}>
        <CustomDayButton day={day} modifiers={modifiers} onSelect={onSelect} />
      </td>
    );
  };
  ...
  const CustomMonths = ({ monthGridProps, onSelect }) => {
    return (
      <div className="flex flex-col">
        {monthGridProps.children[1].props.children.map((month, index) => (
          <div key={index} className="flex flex-row justify-between">
            {month.props.children[1].map((week, weekIndex) => (
              <div key={weekIndex} className="flex gap-1">
                <CustomDayButton {...week.props} onSelect={onSelect} />
              </div>
            ))}
          </div>
        ))}
    ...
    <>
      <DayPicker
        ...
        classNames={{
          ...
          month:
            'flex flex-col p-2 gap-1 text-center border border-gray-300 rounded-md w-full',
        ...
        components={{
          ...
          Day: (singleDayProps) => <Day {...singleDayProps} onSelect={handleSelect} />,
          Weekdays: () => null,
          Months: (monthsProps) => (
            <>
              <div className={monthsClassName}>
                {monthsProps.children.map((months, monthIndex) => {
                  if (!months) {
                    return null;
                  }
                  return (
                    <React.Fragment key={monthIndex}>
                      {months.map((month, innerMonthIndex) => (
                        <React.Fragment key={innerMonthIndex}>
                          {innerMonthIndex > 0 && (
